      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-between">
            <div className="text-sm font-medium text-slate-700">{t("currentStage.selectHint")}</div>
            <button
              type="button"
      ...
      {showConfirm ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4">
          <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#9e1b2b]">{t("currentStage.confirmationRequired")}</p>
            <h3 className="mt-3 text-2xl font-bold text-[#0d2d4f]">{t("currentStage.finalSubmission")}</h3>
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setShowConfirm(false)} className="rounded-xl border border-slate-300 bg-white px-4 py-3 font-semibold text-slate-700">{t("currentStage.cancelButton")}</button>
              <button
                type="button"
              ...
              >
                {isSubmitting ? t("currentStage.submittingLabel") : t("currentStage.confirmButton")}
              </button>
            </div>
          </div>
        </div>
